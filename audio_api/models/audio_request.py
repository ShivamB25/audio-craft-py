...
class VoiceConfig(BaseModel):
    voice_name: VoiceName = Field(
        default=VoiceName.KORE, description="Voice name for TTS"
    )
    speed: Optional[float] = Field(
        default=None, ge=0.1, le=3.0, description="Speech speed (controlled via prompt)"
    )
    pitch: Optional[float] = Field(
        default=None, ge=0.1, le=2.0, description="Speech pitch (controlled via prompt)"
    )
...
class SpeakerConfig(BaseModel):
    ...
    speaker_name: str = Field(
        ..., description="Name of the speaker (must match name in text)"
    )
...
class AudioRequest(BaseModel):
    ...
    model: VoiceModel = Field(
        default_factory=lambda: VoiceModel(
            os.getenv("DEFAULT_TTS_MODEL", "gemini-2.5-pro-preview-tts")
        ),
    ...
    voice_config: Optional[VoiceConfig] = Field(
        ...
        description="Voice configuration for single speaker mode",
    )
    multi_speaker_config: Optional[MultiSpeakerConfig] = Field(
        default=None,
        description="Multi-speaker configuration (required when speaker_mode=MULTIPLE)",
    )
