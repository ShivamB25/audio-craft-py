    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        try:
            ...
            if result.success:
                ...
                results.append({"language": test_case['name'], "status": "SUCCESS", "file": result.file_path})
            else:
                print(f"❌ {test_case['name']}: FAILED")
                print(f"   🚫 Error: {result.error}")
                results.append({"language": test_case['name'], "status": "FAILED", "error": result.error})
                
        except Exception as e:
            print(f"❌ {test_case['name']}: EXCEPTION")
            print(f"   🚫 Exception: {str(e)}")
            results.append({"language": test_case['name'], "status": "EXCEPTION", "error": str(e)})
